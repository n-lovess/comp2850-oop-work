...
class TimeTest : StringSpec({
    ...
    "Exception if hours are invalid" {
        withClue("-1") { shouldThrow<IllegalArgumentException> { Time(-1, 0, 0) } }
        withClue("24") { shouldThrow<IllegalArgumentException> { Time(24, 0, 0) } }
    }

    "Exception if minutes are invalid" {
        withClue("-1") { shouldThrow<IllegalArgumentException> { Time(0, -1, 0) } }
        withClue("60") { shouldThrow<IllegalArgumentException> { Time(0, 60, 0) } }
    }

    "Exception if seconds are invalid" {
        withClue("-1") { shouldThrow<IllegalArgumentException> { Time(0, 0, -1) } }
        withClue("60") { shouldThrow<IllegalArgumentException> { Time(0, 0, 60) } }
    }
